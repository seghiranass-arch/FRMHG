...
export class EquipmentController {
  ...
  private static readonly movementInclude = {
    equipment: true,
    fromOrg: true,
    toOrg: true,
    toMember: true,
  ...
  private static readonly movementHistoryInclude = {
    movement: {
      include: {
        equipment: true,
        fromOrg: true,
  ...
  private static readonly itemMap = (item: Prisma.EquipmentItemGetPayload<{
    include: typeof EquipmentController.itemInclude;
  }>) => ({
    id: item.id,
    name: item.name,
  ...
  private static readonly movementMap = (movement: Prisma.EquipmentMovementGetPayload<{
    include: typeof EquipmentController.movementInclude;
  }>) => {
  ...
  private static readonly historyMap = (entry: Prisma.EquipmentMovementHistoryGetPayload<{
    include: typeof EquipmentController.movementHistoryInclude;
  }>) => ({
    id: entry.id,
  ...
  async listItems() {
    ...
    return items.map(EquipmentController.itemMap);
  }
  ...
  async createMovement(
    ...
    return EquipmentController.movementMap(movement);
  }
  ...
  async listMovements(@CurrentUser() user: AuthUser) {
    ...
    return movements.map(EquipmentController.movementMap);
  }
  ...
  async listMovementHistory(@CurrentUser() user: AuthUser) {
    ...
    return history.map(EquipmentController.historyMap);
  }
}
