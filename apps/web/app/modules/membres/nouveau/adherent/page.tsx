  ...
  const handleSubmit = async (data: MemberFormData) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        setError(text || "Erreur lors de la création de l'adhérent");
        return;
      }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-2xl font-bold text-gray-800">Nouvel Adhérent École Hockey</h1>
              <p className="text-sm text-gray-500">
                Créer un nouveau profil d'adhérent à l'école de hockey
              </p>
            </div>
          </div>
        </div>
      ...
      <div className="bg-green-50 border border-green-200 rounded-xl p-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="font-semibold text-green-800">Adhérent École Hockey</h3>
            <p className="text-sm text-green-700 mt-1">
              Un adhérent est un membre non affilié à un club. Il peut participer aux activités fédérales 
              et doit souscrire un abonnement individuel à l'école de hockey.
            </p>
          </div>
        </div>
      </div>
      ...
      {/* Form - Pre-filled with adherent status */}
      <MemberForm
        ...
        initialData={{ memberStatus: "adherent", licenseStatus: "pending_approval", licenseType: "adherent" }} // Force adherent status
        hideMemberStatusSelection={true} // Hide member status selection
        hiddenTabs={["sports", "club", "documents", "license"]} // Hide specified tabs for adherent
        hiddenSportsFields={[]} // Show all sports fields for adherent
      />
    </div>
  );
}
